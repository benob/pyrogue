...
Pyrogue is a python interpreter based on [micropython](https://docs.micropython.org/en/latest/reference/index.html) with a minimalist library for making roguelikes.

Pyrogue comes with the `rl` module which covers basic roguelike functionalities.
In addition, it comes with the following micropython modules: [math](https://docs.micropython.org/en/latest/library/math.html), [sys](https://docs.micropython.org/en/latest/library/math.html), [ujson](https://docs.micropython.org/en/latest/library/ujson.html), [ure](https://docs.micropython.org/en/latest/library/ure.html).
...
Roll `n` dices of a given type following a string specification.
...
Set the location of resources for loading scripts and assets. Generally, the interpreter already sets that correct value according to how the script was loaded. Currently supports directories and zips and the main exe. Directories must end with a '/', zips must end with '.zip', otherwise it resources are looked for at the end of the specified exe.
...
```python
rl.init('snake', 320, 240)
```

### `rl.load_font(filename, size)`

Load a TTF font for writing text on the display. The text renderer is fast but does not support sub-pixel positionning of letters, which sometimes leads to weird kernings. Only one font of a single size can be loaded at a given time.

```python
rl.load_font('monospace.ttf', 14)
```

### `rl.load_image(index, filename, tile_width=8, tile_height=8)`

Load a png/jpg image in the image registery index. Images are identified by an index from 0 to 31. If an image was already loaded at that index, it is overwritten. Optionnaly, the size of tiles can be specified. It defaults to 8x8.

```python
rl.load_iamge(0, 'tiles.png', 16, 16)
```
...
```python
rl.draw_image(0, 50, 50)
```

### `rl.draw_tile(image, x, y, tile)`
...
### `rl.colorize_tile(image, x, y, tile, fg=None, bg=None)`

Draw a tile from an image representing a tileset. Works the same way as `rl.draw_tile()` but in addition, the background is first painted with `bg` color, and the non-transparent pixels of the tile are colorized with `fg` color. Each can be `None` to skip colorization. 

```python
rl.colorize_tile(0, 5, 5, 17, fg=rl.BLUE, bg=rl.RED)
```

### `rl.print_text(x, y, text, color, align=rl.ALIGN_LEFT, image=None)`

Draw text on the screen at coordinates (x, y) using the given color. The align parameter selects the anchoring point of the text compared to the coordinates (x, y). Valid values are `rl.ALIGN_LEFT`, `rl.ALIGN_RIGHT` and `rl.ALIGN_CENTER`.

By default, loaded TTF font is used. If an image index is passed, then characters of the text are interepreted as tile indexes according to the ASCII code and drawn similarly to `rl.draw_tile()`.

Note that only characters from 32 to 127 can be printed (both when using TTF or a tileset).

### `rl.size_text(text)`

Compute the width and height of text according to the loaded font.

```python
width, height = rl.size_text('hello world')
```
...
Pyrogue's display functions are only honored if called from an update callback which is called every time the screen needs to be repainted. `rl.run()` is a blocking function which calls the update function repeatedly according to the given event filter.
...
* `rl.MOUSE` is received when a mouse event occured.
...
Returns the coordinates of the mouse, as well as the button being pressed. The coordinates are in screen coordinates as defined by `rl.init()`
...
Arrays are fixed-size 2D data structures which allow access to individual elements and have a number of facilities for performing bulk operations. The class also has a number of roguelike-focused methods such as computation of field of vision or path finding.
